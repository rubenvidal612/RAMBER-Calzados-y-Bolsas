...
async function canManage() { const { requirePermission } = await import("@/app/admin-auth"); return requirePermission("MANAGE_BRANCHES"); }

export async function GET() {
  if (!(await canManage())) return NextResponse.json({ error: "Sin permiso para administrar sucursales." }, { status: 403 });
