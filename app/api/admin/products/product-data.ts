...
export type BranchStockInput = { size?: string; branchId?: string; quantity?: number };
export type ProductPayload = {
  ...
  discountValue?: number | null; primaryImageUrl?: string; imageUrls?: string[]; sizes?: ProductSizeInput[]; branchStocks?: BranchStockInput[];
...
export type CleanProduct = {
  ...
  imageUrls: string[]; sizes: Array<{ size: string; quantity: number }>; branchStocks: Array<{ size: string; branchId: string; quantity: number }>;
  primaryImageZoom: number; primaryImageX: number; primaryImageY: number;
};
...
export function cleanProduct(body: ProductPayload): CleanProduct | null {
  ...
  const suppliedStock = (body.branchStocks || []).map((item) => ({ size: String(item.size || "").trim(), branchId: String(item.branchId || "").trim(), quantity: Number(item.quantity) })).filter((item) => item.size && item.branchId);
  const branchStocks = suppliedStock.length ? suppliedStock : sizes.map((item) => ({ size: item.size, branchId: "crystal", quantity: item.quantity }));
  ...
  if (!body.category || !["dama", "infantil"].includes(body.category) || !name || !model || !body.color?.trim() || !Number.isInteger(costPrice) || costPrice < 0 || !Number.isInteger(publicPrice) || publicPrice < 0 || !sizes.length || sizes.some((item) => !Number.isInteger(item.quantity) || item.quantity < 0) || branchStocks.some((item) => !Number.isInteger(item.quantity) || item.quantity < 0) || !body.primaryImageUrl || !imageUrls.length || primaryImageZoom < 100 || primaryImageZoom > 300 || primaryImageX < 0 || primaryImageX > 100 || primaryImageY < 0 || primaryImageY > 100) return null;
  ...
  return { category: body.category, model, sku: body.sku?.trim() || null, name, description: body.description?.trim() || "", color: body.color.trim(), costPrice, publicPrice, promoPrice, inOffer, discountType, discountValue, primaryImageUrl: body.primaryImageUrl, imageUrls, sizes, branchStocks, primaryImageZoom, primaryImageX, primaryImageY };
}
